...
fs.writeFileSync(
  path.join(process.cwd(), "json_db_storage_two", "sampleTwo.json"),
  JSON.stringify([
    { slug: "tree", name: "flurgl" },
    { slug: "post", name: "fred" },
  ])
);
...
describe("JsonDB('sampleTwo'), slug, json_db_storage_two", () => {
  ...
  test("delete by id in db", () => {
    expect(dbTwo.deleteById("plants")).toBe(true);
  });

  test("delete by id, not in db", () => {
    expect(dbTwo.deleteById("plants")).toBe(false);
  });
